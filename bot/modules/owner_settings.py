    ...
    if data[1] == "env":
        if data[2] == "editenv" and STATE == 'edit':
            ...
            if data[3] in default_values:
                ...
                srun(["pkill", "-9", "-f", "gunicorn"])
                Popen(["gunicorn", "web.server:app", "--bind=0.0.0.0:80", "--logger-class=web.log_config.StubbedGunicornLogger"])
        ...
        if data[2] == 'aria_menu':
            ...
            if aria2_defaults[data[3]] == aria2_options[data[3]]:
                await query.answer(text='Value already same as you added in aria2.sh!', show_alert= True)
                return
            await query.answer()
            ...
            try:
                if "/ignore" in response.text:
                    ...
                    if value.lower() == 'true':
                        ...
                        value = int(value)
                        srun(["pkill", "-9", "-f", "gunicorn"])
                        Popen(["gunicorn", "web.server:app", f"--bind=0.0.0.0:{value}", "--logger-class=web.log_config.StubbedGunicornLogger"])
